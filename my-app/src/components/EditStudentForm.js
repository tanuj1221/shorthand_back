...
const EditStudentForm = () => {
  ...
  useEffect(() => {
    if (studentId) {
      axios.get(`http://13.234.111.187 :3000/students/details/${studentId}`)
        .then(response => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.put(`http://13.234.111.187 :3000/students/${studentId}`, updatedStudentDetails);
