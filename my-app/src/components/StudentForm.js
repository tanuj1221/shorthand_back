...
function StudentForm() {
  ...
  useEffect(() => {
    const fetchBatchInfo = async () => {
      const result = await axios('http://15.206.159.230:3000/batch');
  ...
  useEffect(() => {
    if (studentId) { // Only fetch if studentId is available
      const fetchStudentData = async () => {
        try {
          const response = await axios.get(`http://15.206.159.230:3000/students/${studentId}`);
  ...
  useEffect(() => {
    const fetchSemesters = async () => {
      const result = await axios(`http://15.206.159.230:3000/batch?batch_year=${selectedYear}`);
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      const result = await axios('http://15.206.159.230:3000/subjectsAndIds');
  ...
  const handleSubmit = async (event) => {
    ...
    for (const subjectId of selectedSubjects) {
      ...
      try {
        await axios.post('http://15.206.159.230:3000/registerstudent', newStudent);
