...
function StudentForm() {
  ...
  useEffect(() => {
    if (studentId) { // Only fetch if studentId is available
      const fetchStudentData = async () => {
        try {
          const response = await axios.get(`http://65.0.31.147:3000/students/${studentId}`);
  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      const result = await axios('http://65.0.31.147:3000/subjectsAndIds');
  ...
  const handleSubmit = async (event) => {
    ...
    for (const subjectId of selectedSubjects) {
      ...
      try {
        await axios.post('http://65.0.31.147:3000/registerstudent', newStudent);
