...
function StudentForm() {
    ...
    useEffect(() => {
        const fetchBatchInfo = async () => {
            const result = await axios('http://13.235.95.5:3001/batch');
    ...
    useEffect(() => {
        const fetchSemesters = async () => {
            const result = await axios(`http://13.235.95.5:3001/batch?batch_year=${selectedYear}`);
    ...
    useEffect(() => {
        const fetchSubjects = async () => {
            const result = await axios('http://13.235.95.5:3001/subjectsAndIds');
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            await axios.post('http://13.235.95.5:3001/registerstudent', newStudent);
